	...
	<view>
		<uni-popup ref="vip" type="bottom">
			<view class="bg-FFFFFF hidden" style="border-radius: 24rpx 24rpx 0 0;padding: 48rpx 28rpx 0 28rpx;">
				...
				<!-- 标题 -->
				<view class="flex flex-y-end fs-32 ">
					<view class="bold" style="color: #C2915A;padding-left: 11rpx;">充值条数</view>
					<view class="fs-26 color-999999" style="line-height: 38rpx;margin-left: 28rpx;">充值条数一个月后过期清零</view>
				</view>
				<!-- 标题 -->
				...
				<view style="margin-top: 48rpx;">
					<view @tap.stop="choose_one(item,index)" v-for="(item,index) in list" :key="index" class="vip-item"
						:style="{animationDuration:0.3 + 0.14 * index + 's' }" :class="item.checked?'active':''">
						<view>
				...
				<view style="margin-bottom: 50rpx;" class="buy-vip-bar">
					<view style="padding-left: 60rpx;height: 100%;" class="flex-1 flex flex-y-center">{{checkedItem?(`${checkedItem.amt}元${checkedItem.number}条`):''}}</view>
					<view @tap.stop="less(buy_now)" hover-class="buy-vip-btn-width" hover-start-time="1"
						hover-stay-time="300" class="bold fs-28 flex flex-x-y bg-all buy-vip-btn">立即购买</view>
				</view>
				<!-- 购买 -->
