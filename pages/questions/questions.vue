<template>
	<view>
		<custom-header background="#F6F6F6" :tabsIndex="1"></custom-header>


		<!-- 剩余条数 -->
		<residue-number @clear="clear_questions"></residue-number>
		<!-- 剩余条数 -->
		<view class="flex">
		...
		<bottomInput @send="add" ref="bottomInput" @heightChange="bottomInputChange"></bottomInput>
...
<script>
	...
	export default {
		...
		onLoad() {
			...
			this.oldAvatar = this.user_info.avatarUrl;
		},
		onUnload() {
		...
		onShow() {
			...
			this.getLastSerach();
			if (this.oldAvatar != this.user_info.avatarUrl) {
				this.list.forEach(val => {
					if (val.isFromUser) {

						val.avatar = this.user_info.avatarUrl;
					}
				});
				this.oldAvatar = this.user_info.avatarUrl;
			}
